...
export class StoreList extends React.Component {
    ...
    render() {
        ...
        return <div>
            <h1>Store Record</h1>
            <p>
                <a href="addStore"><button style={{ backgroundColor: "blue" }}>Create New</button></a>
            </p>
            {contents}
        </div>;
    }
            ...
            <tbody>
                {storeListData.map(item =>
                    <tr key={item.storeId}>
                        ...
                        <td ><button className="action" style={{ backgroundColor: "yellow" }} onClick={(id) => this.FuncEdit(item.storeId)}>Edit</button></td>
                        <td ><button className="action" style={{ backgroundColor: "red" }} onClick={(id) => this.FuncDelete(item.storeId)}>Delete</button></td>
