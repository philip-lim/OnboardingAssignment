...
export class CustomerList extends React.Component {
   ...
   render() {
        ...
        return <div>
            ...
            <p>
                <a href="addCustomer"><button style={{ backgroundColor: "blue" }}>Create New</button></a>
            </p>
            {contents}
        </div>;
    }
            ...
            <tbody>
                {customerListData.map(item =>
                    <tr key={item.customerId}>
                        ...
                        <td ><button className="action" style={{ backgroundColor: "yellow" }}onClick={(id) => this.FuncEdit(item.customerId)}>Edit</button></td>
                        <td ><button className="action" style={{ backgroundColor: "red" }} onClick={(id) => this.FuncDelete(item.customerId)}>Delete</button></td>
                    </tr>
                )}
            </tbody>
        </table>;
